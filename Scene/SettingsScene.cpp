...
bool SettingsScene::canUseCheatCode = true; 
...
void SettingsScene::Initialize() {
    ...
    Engine::ImageButton* Modebtn = new Engine::ImageButton("stage-select/dirt.png", "stage-select/floor.png", halfW - 200, halfH * 3 / 2 - 450, 400, 100);
    Modebtn->SetOnClickCallback(std::bind(&SettingsScene::ClickChangeMode, this, 1));
    AddNewControlObject(Modebtn);
    text = new Engine::Label(canUseCheatCode ? "Code Opened" : "Code Closed", "pirulen.ttf", 36, halfW, halfH * 3 / 2 - 400, 0, 0, 0, 255, 0.5, 0.5);
    AddNewObject(text);
}
...
bool SettingsScene::isEasyMode() {
    return canUseCheatCode;
}

void SettingsScene::ClickChangeMode(int stage) {
    canUseCheatCode = !canUseCheatCode;
}